...
lambda = 6.0 ;% average cal1 arrival rate (times / hour)
ht = 120.0;%average call holding time isecond)
...
output = zeros (4,5); % output natrix
...
there = userposi - baseinfo...
    (othercell, :) + baseinfo(around, :) + baseinfo(numcell, : ) ;
...
4)== 1 & userinfo(othercell,other,6)==ch...
userposi(1,1:2) = userinfo...
(othercell,other,1: 2);
here = baseinfo(numcell,: );
there = userposi - baseinfo...
...
% 绘制图像
figure;
plot(meshposition(:,1),meshposition(:,2),'o');
xlabel('x');
ylabel('y');
